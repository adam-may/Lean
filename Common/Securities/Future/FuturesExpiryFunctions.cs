...
namespace QuantConnect.Securities.Future
{
    ...
    public class FuturesExpiryFunctions
    {
        ...
        public static Dictionary<string, Func<DateTime, DateTime>> FuturesExpiryDictionary = new Dictionary<string, Func<DateTime, DateTime>>()
        {
            ...
            {Futures.Financials.GBL, (time =>
                {
                    // Delivery day
                    // The tenth calendar day of the respective quarterly month, if this day is an exchange day;
                    ...
                    var holidays = MarketHoursDatabase.FromDataFolder()
                        .GetEntry(Market.DTB, Futures.Financials.GBL, SecurityType.Future)
                        .ExchangeHours
                        .Holidays;

                    var deliveryDate = time.AddDays(9);

                    if (!FuturesExpiryUtilityFunctions.NotHoliday(deliveryDate, holidays))
                    {
                        deliveryDate = FuturesExpiryUtilityFunctions.AddBusinessDays(deliveryDate, 1, t => FuturesExpiryUtilityFunctions.NotHoliday(t, holidays));
                    }

                    return FuturesExpiryUtilityFunctions.AddBusinessDays(deliveryDate, -2, t => FuturesExpiryUtilityFunctions.NotHoliday(t)).Add(new TimeSpan(12, 30, 0));
                })
            },

            {Futures.Financials.SG, (time =>
                {
                    // Last Trading Day
                    // Two business days prior to the last business day in the delivery month.
                    // On the Last Trading Day, trading in the front delivery month will cease at 11:00
                    
                    var holidays = MarketHoursDatabase.FromDataFolder()
                        .GetEntry(Market.ICEEU, Futures.Financials.SG, SecurityType.Future)
                        .ExchangeHours
                        .Holidays;

                    var lastBusinessDay = FuturesExpiryUtilityFunctions.NthLastBusinessDay(time, 3, holidays);
                    return lastBusinessDay.Add(new TimeSpan(11, 0, 0));
                })
            },
